...
app.post("/api/auth/google", async (req, res) => {
  ...
  try {
    ...
    if (!user) {
      ...
      const password_hash = await bcrypt.hash(`google-${googleId}-${Date.now()}`, salt);
...
app.post("/api/auth/register", async (req, res) => {
  ...
  try {
    ...
    console.error(err);
    res.status(500).json({ error: "Internal server error. Username or email might exist." });
  }
});
...
// VENDOR ROUTES
// ==========================================
app.get("/api/vendor/products", authenticateToken, requireRole('vendor'), async (req, res) => {
  try {
    const result = await db.query("SELECT * FROM products WHERE vendor_id = $1 ORDER BY id DESC", [req.user.id]);
    res.json(result.rows);
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: "Internal server error" });
...
app.post("/api/vendor/products", authenticateToken, requireRole('vendor'), async (req, res) => {
...
app.delete("/api/vendor/products/:id", authenticateToken, requireRole('vendor'), async (req, res) => {
  try {
    const result = await db.query("DELETE FROM products WHERE id = $1 AND vendor_id = $2 RETURNING *", [req.params.id, req.user.id]);
    if (result.rows.length > 0) {
...
// ==========================================
app.get("/api/vendor/orders", authenticateToken, requireRole('vendor'), async (req, res) => {
  try {
    const result = await db.query(`
...
app.get("/api/vendor/earnings", authenticateToken, requireRole('vendor'), async (req, res) => {
  try {
    const result = await db.query(`
...
app.post("/api/cart", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    const existing = await db.query("SELECT id FROM cart_items WHERE customer_id = $1 AND product_id = $2", [req.user.id, product_id]);
    if (existing.rows.length > 0) {
      // Update quantity
      await db.query("UPDATE cart_items SET quantity = quantity + $1 WHERE id = $2", [quantity || 1, existing.rows[0].id]);
    } else {
      // Insert new
      await db.query("INSERT INTO cart_items (customer_id, product_id, quantity) VALUES ($1, $2, $3)", [req.user.id, product_id, quantity || 1]);
    }
...
app.post("/api/orders", authenticateToken, async (req, res) => {
  try {
    ...
    // 4. Clear cart
    await db.query("DELETE FROM cart_items WHERE customer_id = $1", [req.user.id]);
